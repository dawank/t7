...
class LINE extends LineAPI {
    ...
    get myBot() {
        const bot = ['ua33a5b61b096902826ad88ceec5cab56'];
        return bot; 
    }
    ...
    poll(operation) {
     ...
     {
let tokenku = new Message();
tokenku.to = operation.param1;
tokenku.text = "Woooiii!!! Jangan Main Kick Sembarangan!!! Aku Kick Kamu Yaa, Byeee!!!"
this._client.sendMessage(0,tokenku);
     }
            // op1 = group nya
            // op2 = yang 'nge' kick
            // op3 = yang 'di' kick
...
token.to = operation.param1;
token.text = "Jangan Ubah Gambar Dan Nama Group/Jangan Dimainin QR-nya -__- Aku Kick Yaa !"+"Cuma Admin Yang Bisa!!!"
this._client.sendMessage(0,token);
     }
            if(!this.isAdminOrBot(operation.param2)){
                this._kickMember(operation.param1,[operation.param2]);
            } 
...
let token = new Message();
token.to = operation.param1;
token.text = "Byee~Byee... Jangan Balek Lagi Yaa Ke Group... Bikin Semak Aja... Makasih ^_^"
...
let tokenku = new Message();
tokenku.to = operation.param1;
tokenku.text = "Terima Kasih Telah Invite Saya Di Group Anda ^_^\n\nSilahkan Ketik [/help] Untuk Mengetahui Command Bot Kami.\n\n~VIPRO~"
,"Invite Juga Token II Kesini ^_^"
...
let token = new Message();
token.to = operation.param1;
token.text = "Invite Juga Token II Kesini ^_^"
...
let tokenku = new Message();
tokenku.to = operation.param1;
tokenku.text = "Hallo Selamat Datang ^_^ Jangan Nakal Yaa Di Group Ini ^_^"
...
let tokenku = new Message();
tokenku.to = operation.param1;
tokenku.text = "Kok Dibatalin? Emangnya Dia Siapa?"
...
let tokenku = new Message();
tokenku.to = operation.param1;
tokenku.text = "Thx For Add Me 😘 Invite Me To Your Group ^_^"
this._client.sendMessage(0,tokenku);
}
...
let tokenku = new Message();
tokenku.to = operation.param1;
tokenku.text = "Jangan Main Invite Sembarangan... Siapa Tau Kicker 😧"
...
this.command('Halo', ['halo disini Token','kamu siapa?']);
this.command('Hi', ['hi disini Token','kamu siapa?']);

this.command('Token', ['Apa kak? ada yg bisa dibantu?','Ketik /help untuk bantuan']);
